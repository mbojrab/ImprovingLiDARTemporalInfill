...
class _FinalBlock(_LayerSequence):
    ...
    def __init__(self, kernels, kernel_size, strides=1, l2_reg=0.0, name=''):
        ...
        self._seq.extend([tf.keras.layers.Conv2DTranspose(filters=kernels, kernel_size=kernel_size,
                          ...
                          _GatedConv2D(kernels, 5, 1, l2_reg, activation=tf.keras.layers.LeakyReLU())])
...
class _Decoder(_LayerSequence):
    ...
    def __init__(self, num_channels, kernels, kernel_size, strides=None, l2_reg=0.0):
        ...
        self.squash_layer = _GatedConv2D(num_channels, 1, 1, l2_reg, _SoftPlus())
    ...
    def call(self, features):
        ...
        out = tf.squeeze(features[0], axis=0)
        for layer, residual in zip(self._seq, features[1:]):
            out = tf.concat((layer(out), tf.squeeze(residual, axis=0)), axis=-1)
