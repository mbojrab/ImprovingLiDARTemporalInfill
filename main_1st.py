...
if __name__ == '__main__':
    ...
    # create the model
    options.model_dir = os.path.join(options.model_dir, '1')
    tf.summary.trace_on()
    if os.path.exists(os.path.join(options.model_dir, 'saved_model.pb')):
        ...
        generator = command_line.unet_factory(options=options)(options=options)
    ...
    pipeline = pipeline.StageOneTemporalInfillPipeline(strides=options.strides, unoccluded=10, target_size=(None, 96))

    # choose the appropriate training routine
    train.train_model(generator, discriminator, pipeline, options, tf.losses.Huber(delta=10, reduction=tf.keras.losses.Reduction.NONE))
