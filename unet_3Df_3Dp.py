...
class _GatedConv2D(_LayerSequence):
    ...
    def call(self, features):
        if len(features.shape) > 4:
            out = tf.squeeze(features, axis=0)
        else:
            out = features
        out = self._seq[0](out) * self._seq[1](out)
        return tf.expand_dims(out, axis=0) if len(features.shape) > 4 else out
...
class _GatedConv3D(_LayerSequence):
    ...
    def call(self, features):
        if len(features.shape) < 5:
            out = tf.expand_dims(features, axis=0)
        else:
            out = features
        out = self._seq[0](out) * self._seq[1](out)
        return tf.squeeze(out, axis=0) if len(features.shape) < 5 else out
...
class _BlockSequence(_LayerSequence):
    ...
    def __init__(self, kernels, kernel_size, strides=1, l2_reg=0.0, activation=None, name=''):
        ...
        self._seq.extend([tf.keras.layers.Conv3DTranspose(filters=kernels, kernel_size=kernel_size_3D,
                          ...
                          if strides != 1 else
                          _GatedConv2D(kernels, kernel_size, strides, l2_reg, activation)] +
                          [tf.keras.layers.BatchNormalization(),
                           activation,
                           _GatedConv2D(kernels, 5, 1, l2_reg, None),
                           tf.keras.layers.BatchNormalization(),
                           activation,
                           _GatedConv2D(kernels, 5, 1, l2_reg, None),
                           tf.keras.layers.BatchNormalization(),
...
class _Encoder(_LayerSequence):
    ...
    def __init__(self, channels, kernels, kernel_size, strides=None, l2_reg=0.0, name=None):
        ...
        self._seq.extend([_GatedConv3D(k, ks, s, l2_reg, tf.keras.layers.LeakyReLU())
...
class _Decoder(_LayerSequence):
    ...
    def call(self, features):
        def squasher(o):
            #return tf.squeeze(tf.clip_by_value(o, 0., 1.), axis=0)
